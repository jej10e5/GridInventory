...
{
    public UIGrideScrollViewDirector director;
    ...
    void Start() 
    {
        ...
        Debug.LogFormat("Exits: {0}", File.Exists(path));
        if(File.Exists(path))
        {
            //있으면 : 기존유저 -> 역질렬화 -> InfoManager.instance.inventoryInfo (변수에 저장(메모리 올리기))
        ...
        {
            //없으면 : 신규유저 -> info 만들어서 파일 inventory_info.json 저장 -> InfoManager.instance.inventoryInfo (변수에 저장(메모리 올리기))
